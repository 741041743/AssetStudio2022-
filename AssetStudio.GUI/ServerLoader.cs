    ...
    public class ServerLoader
    {
        ...
        public static async Task<string> LoadFromServer(string url, string inputVersion, IProgress<int> progress = null)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    Logger.Info($"正在请求服务器: {url}");

                    // 请求URL获取JSON内容
                    var response = await httpClient.GetStringAsync(url);

                    Logger.Info("成功获取服务器响应");
                    ...
                    var serverVersion = jsonObject["version"]?.ToString();

                    if (string.IsNullOrEmpty(resourceUrl))
                    {
                        Logger.Error("JSON中未找到resource_url字段");
                        return null;
                    }
                    
                    if (string.IsNullOrEmpty(serverVersion))
                    {
                        Logger.Error("JSON中未找到version字段");
                        return null;
                    }

                    Logger.Info($"解析成功 - resource_url: {resourceUrl}, server_version: {serverVersion}");

                    // 替换resource_url中的$file_ver占位符
                    var finalUrl = resourceUrl.Replace("$file_ver", inputVersion);

                    Logger.Info($"最终资源URL: {finalUrl}");

                    // 在应用程序根目录创建本地缓存目录
                    var appRootPath = AppDomain.CurrentDomain.BaseDirectory;
                    var serverCachePath = Path.Combine(appRootPath, "ServerCache");
                    var resourcesFolder = Path.Combine(serverCachePath, $"{inputVersion}_full");

                    Logger.Info($"本地缓存目录: {resourcesFolder}");

                    // 如果目录已存在，先删除旧缓存
                    if (Directory.Exists(resourcesFolder))
                    {
                        Logger.Info($"检测到已存在的缓存目录，正在删除: {resourcesFolder}");
                        try
                        {
                            Directory.Delete(resourcesFolder, true);
                            Logger.Info("旧缓存目录已删除");
                        }
                        catch (Exception ex)
                        {
                            Logger.Error($"删除旧缓存目录失败: {ex.Message}");
                            return null;
                        }
                    }
                    ...
                    Logger.Info("已创建新缓存目录");

                    // 下载资源列表文件
                    Logger.Info("开始下载资源列表文件...");
                    var filesListUrl = finalUrl.TrimEnd('/') + "/files.txt";
                    ...
                    {
                        Logger.Error("解析files.txt失败");
                        return null;
                    }
                    
                    Logger.Info($"资源列表包含 {newHashCheck.list.Count} 个文件");
                    
                    // 获取基础资源URL（去掉$file_ver占位符）
                    var baseResourceUrl = resourceUrl.TrimEnd('/').Replace("$file_ver", "");
                    
                    await DownloadResourcesParallel(newHashCheck.list, newHashCheck, int.Parse(inputVersion), $"{inputVersion}_full", serverCachePath, baseResourceUrl, progress);
            ...
            {
                Logger.Error($"服务器加载失败: {ex.Message}");
                return null;
            }
        }
        ...
        static async Task DownloadResourcesParallel<T>(Dictionary<string, T> resourcesSet, hashCheck resourcesHashCheck, int version, string versionFolder, string tempResourceFolder, string baseServerResourcePath, IProgress<int> progress = null)
        {
            ...
            try
            {
                ...
                Logger.Info($"开始并行下载 {resourcesSet.Count} 个资源文件...");
                ...
                await Task.Run(() =>
                {
                    Parallel.ForEach(resourcesSet, parallelOptions, (keyPair, loopState) =>
                    {
                        ...
                        {
                            Logger.Error($"下载 {abName} 失败: {ex.Message}");
                        }
                        finally
                        {
                            lock (parallelDownloadInfo)
                            {
                                ...
                                {
                                    Logger.Info($"下载进度: {parallelDownloadInfo.currentIdx}/{parallelDownloadInfo.totalCount} ({progressValue}%)");
                                }
                            }
                        }
                    });
                ...
                stopWatch.Stop();
                Logger.Info($"下载 [{version}] 版本资源完成，耗时: {stopWatch.ElapsedMilliseconds} ms ({stopWatch.Elapsed.TotalSeconds:F2} 秒)");
            }
            catch (Exception e)
            {
                Logger.Error($"并行下载失败: {e.Message}");
                throw;
            }
            finally
            {
        ...
        private static void DownloadFileSync(string fileUrl, string savePath)
        {
            ...
            {
                Logger.Error($"下载文件失败 {Path.GetFileName(savePath)}: {ex.Message}");
                throw;
            }
        }
        ...
        public static async Task DownloadFileAsync(string fileUrl, string savePath)
        {
            try
            {
                ...
                using (var client = new WebClient())
                {
                    ...
                    Logger.Info($"下载完成: {Path.GetFileName(savePath)}");
                }
            }
            catch (Exception ex)
            {
                Logger.Error($"下载文件失败 {fileUrl}: {ex.Message}");
                throw;
            }
        }
    }
